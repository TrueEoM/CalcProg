...
Grupo nr. & 10
\\\hline
a89486 & Tomas Ferreira de Sousa Dias
\\
a98728 & Vasco Rafael Barroso Gonçalves Rito
\\
a98712 & Pedro Alexandre da Silva Oliveira
\end{tabular}
\end{center}
...
\start
   |lcbr(
        f . in = h . F (split f (split f2 f3)) 
   )(
        f2 . in = k . F (split f (split f2 f3)) 
   )(
        f3 . in = h . F (split f (split f2 f3)) 
   )|
%
\just\equiv{ |in = either (const 0) succ| }
%
    |lcbr(
        f . (either (const 0) succ) = (either h1 h2) . (id + (split f (split f2 f3)))
   )(
        f2 . (either (const 0) succ) = (either k1 k2) . (id + (split f (split f2 f3)))
   )(
        f3 . (either (const 0) succ) = (either l1 l2) . (id + (split f (split f2 f3)))
   )|
%
\just\equiv{ (20), (22), (27), (1) }
%
    |lcbr(
        f . (const 0) = h1
   )(
        f . succ = h2 . (split f (split f2 f3))
   )|
   |lcbr(
        f2 . (const 0) = k1
   )(
        f2 . succ = k2 . (split f (split f2 f3))
   )|
   |lcbr(
        f3 . (const 0) = l1
   )(
        f3 . succ = l2 . (split f (split f2 f3))
   )|
\qed
\end{eqnarray*}

  f2 a b c n = f a b c (n+1)

  f2 a b c (n+1) = f a b c (n+2)

  f3 a b c n = f a b c (n+2) = f2 a b c (n+1)

  f3 a b c (n+1) = f a b c (n+3) 

  = a * f a b c (n+2) + b * f a b c (n+1) + c * f a b c n

  = a * f3 a b c n + b * f2 a b c n + c * f a b c n
...
\start
    ...
    |lcbr(
      ...
      f2 a b c (n+1) = f3 a b c n
    )|

    |lcbr(
      f3 a b c 0 = (const 1)
    )(
      f3 a b c (n+1) = a * f3 a b c n + b * f2 a b c n + c * f a b c n
    )|

\just\equiv{ Convert To Pointfree }
    |lcbr(
      f . (const 0) = (const 0)
    )(
      f . succ = p1 . p2 . (split f (split f2 f3))
    )|

    |lcbr(
      f2 . (const 0) = const 1
    )(
      f2 . succ = p2 . p2 . (split f (split f2 f3))
    )|

    |lcbr(
      f3 . (const 0) = (const 1)
    )(
      f3 . succ = (add . (id >< add). ((c*) >< ((b*) >< (a*)))) . (split f (split f2 f3))
    )|
\qed
\end{eqnarray*}
...
\begin{code}
loop a b c = split (p1 . p2) (split (p2 . p2) (add . (id >< add) . ((c*) >< ((b*) >< (a*)))))
initial = (0, (1, 1))
wrap = p1
...
\begin{eqnarray*}
\xymatrix{
  (S^*)^* & & S+S|><|(S^*)^*\ar@@/_1.5pc/[ll]_{|g|} \\
  |Exp S S|\ar[u]^{|post|}\ar@@/_1.5pc/[rr]_{|outExp|} & & S + S \times (|Exp S S|)^*\ar@@/_1.5pc/[ll]_{|inExp|}\ar[u]_{id + id \times post^*} \\
  S^*\ar@@/_1.5pc/[rr]_{|gene|}\ar[r]^(0.35){|out|}\ar[u]^{|tax|} & S + S \times S^*\ar[r]^(0.45){f} & S + S \times (S^*)^*\ar[u]_{id + id \times tax^*}
}
\end{eqnarray*}

Gene de |tax|:
\begin{code}
gene = (id -|- id >< (groupBy (const $ isPrefixOf "    ") . map (drop 4))) . out

\end{code}
Função de pós-processamento:
\begin{code}
post = delDups . build . tar
  where build = concat . map (\(h,t) -> ((drop 1) . prefixes) (h ++ [t]))
        delDups = map head . group . iSort
...
\begin{eqnarray*}
\xymatrix{
  (Square)^* & & Square|><|(Square^*)^*\ar@@/_1.5pc/[ll]_{|gr2l|} \\
  |Rose Square|\ar[u]^{|rose2List|}\ar@@/_1.5pc/[rr]_{|outRose|} & & Square \times (|Rose Square|)^*\ar@@/_1.5pc/[ll]_{|inRose|}\ar[u]_{|id + rose2List|^*} \\
  Square \times Int\ar@@/_1.5pc/[rr]_{|gsq|}\ar[u]^{|squares|} & & Square \times (Square |><| Int)^*\ar[u]_{|id >< squares|^*}
}
\end{eqnarray*}
...
gsq (((xPos,yPos),side),0) = (((xPos+side/3,yPos+side/3),side/3),[]) 
gsq (((xPos,yPos),side),lvl+1) = 
  (((xPos+(side/3),yPos+(side/3)),side/3), 
  [(((xPos+(side/3)*nX,yPos+(side/3)*nY), side/3),lvl) | (nX,nY) <- mods])
    where
      mods = [(0,0),(0,1),(0,2),(1,0),(2,0),(1,2),(2,2),(2,1)]
...
\begin{eqnarray*}
\xymatrix{
  |IO [()]| & & id + Square^* \times |IO [()]|\ar@@/_1.5pc/[ll]_{|presGene|} \\
  (Square^*)^*\ar[u]^{|present|}\ar@@/_1.5pc/[rr]_{|out|} & & id + Square^* \times (Square^*)^*\ar@@/_1.5pc/[ll]_{|in|}\ar[u]_{|id + id >< present|} \\
  Int\ar@@/_1.5pc/[rr]_{|carpGene|}\ar[u]^{|carpets|} & & id + (Square^* \times Int)\ar[u]_{|id + id >< carpets|}
}
\end{eqnarray*}
...
carpGene = (id -|- f) . grd (<0) 
    where 
      f = split (curry sierpinski ((0,0),32)) pred

present = sequence . reverse' . cataList presGene

presGene = either a b
  where
    a _ = [mempty]
    b (sqr1,sqrl) = (drawSq sqr1 >> await) : sqrl
\end{code}
...
\begin{eqnarray*}
\xymatrix{
  Team & & Team + Team^2\ar@@/_1.5pc/[ll]_{|either id koCriteria|} \\
  LTree Team\ar[u]^{|knockoutStage|}\ar@@/_1.5pc/[rr]_{|out|} & & Team + (LTree Team)^2\ar@@/_1.5pc/[ll]_{|in|}\ar[u]_{|id + knockoutStage|} \\
  Grupo^*\ar@@/_1.5pc/[rr]_{|grupoGene|}\ar[u]^{|groupStage|} & & Team + (Grupo^*)^2 \ar[u]_{id + groupStage^2}
}
\end{eqnarray*}
